...
func (database *PromotionDB) Create(promotion *entity.Promotion) (string, error) {
	stmt, err := database.db.Prepare("INSERT INTO promotions (id, description, min, max, price) VALUES (?,?,?,?,?)")
	if err != nil {
		return "", err
	}
	defer stmt.Close()

	if _, err := stmt.Exec(promotion.ID, promotion.Description, promotion.Min, promotion.Max, promotion.Price); err != nil {
		return "", err
	}
	...
	if err := row.Scan(&p.ID, &p.Description, &p.Min, &p.Max, &p.Price); err != nil {
		return "", err
	}
...
func (database *PromotionDB) Update(id string, promotion *entity.Promotion) (string, error) {
	stmt, err := database.db.Prepare("UPDATE promotions SET description =?, min =?, max =?, price =? WHERE id =?")
	if err != nil {
		return "", err
	}
	defer stmt.Close()

	if _, err := stmt.Exec(promotion.Description, promotion.Min, promotion.Max, promotion.Price, id); err != nil {
		return "", err
	}
...
func (database *PromotionDB) Get(id string) (*entity.Promotion, error) {
	...
	if err := row.Scan(&promotion.ID, &promotion.Description, &promotion.Min, &promotion.Max, &promotion.Price); err != nil {
		return nil, err
	}
...
func (database *PromotionDB) List() ([]entity.Promotion, error) {
	...
	for rows.Next() {
		var promotion entity.Promotion
		err := rows.Scan(&promotion.ID, &promotion.Description, &promotion.Min, &promotion.Max, &promotion.Price)
		if err != nil {
			return nil, err
		}
